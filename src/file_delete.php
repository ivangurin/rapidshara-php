<?php
  ...
  if($go_user && $go_user->is_root())
    $gs_configuration["file_delete"] = true;

  // Check for delete allowed
  if(!$gs_configuration["file_delete"]){
    $gv_title = $gv_data = $go_configuration->get_comment("file_delete");
  ...
  $gv_title = $gv_data = "НЛО удалило файл!";
  require("info.php"); exit;
